...
const Detector = () => {
  ...
  const [videoWidth, setVideoWidth] = useState("480px");

  const createHandLandmarker = async () => {
    const vision = await FilesetResolver.forVisionTasks(
      "https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@0.1.0-alpha-11/wasm"
    );
    setHandLandmarker(await HandLandmarker.createFromOptions(vision, {
      baseOptions: {
        modelAssetPath: `https://storage.googleapis.com/mediapipe-assets/hand_landmarker.task`
      },
      runningMode: runningMode,
  ...
  const enableCam = async (event) => {
    ...
    if (webcamRunning === true) {
      ...
      setEnableWebcamButton("ENABLE PREDICTIONS");
    } else {
      setWebcamRunning(true);
      setEnableWebcamButton("DISABLE PREDICITONS");
    }
